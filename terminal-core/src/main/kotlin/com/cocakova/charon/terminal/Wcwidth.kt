...
/**
 * Cell width of a code point: 0 (combining/format), 1, or 2 (wide).
 *
 * Charon matches what a remote `wcwidth()` thinks — that's what programs lay out
 * against — not perfect grapheme clustering. Tables follow Markus Kuhn's wcwidth
 * semantics with East Asian Wide/Fullwidth ranges plus the modern emoji blocks that
 * contemporary glibc/musl report as wide. Regeneration from UCD data is a documented
 * follow-up (docs/TERMINAL.md); these intervals cover the practical corpus.
 */
object Wcwidth {

    fun of(codePoint: Int): Int = when {
        codePoint < 0x20 -> 0 // controls never reach the grid, but stay total
        codePoint < 0x7F -> 1
    ...
    private fun isZeroWidth(cp: Int): Boolean =
        cp == 0x200B || cp == 0x200C || cp == 0x200D || cp == 0x200E || cp == 0x200F ||
            cp == 0xFEFF || cp == 0x00AD ||
            binarySearch(ZERO, cp)

    private fun isWide(cp: Int): Boolean = binarySearch(WIDE, cp)

    private fun binarySearch(table: IntArray, cp: Int): Boolean {
        var lo = 0
    ...
    // Combining / zero-width intervals (start, end pairs, sorted).
    private val ZERO = intArrayOf(
        ...
        0x17C6, 0x17C6, 0x17C9, 0x17D3, 0x17DD, 0x17DD, 0x180B, 0x180E,
        ...
        0x11CB5, 0x11CB6, 0x16AF0, 0x16AF4, 0x16B30, 0x16B36, 0x16F8F, 0x16F92,
    ...
    // Wide/fullwidth intervals (start, end pairs, sorted).
    private val WIDE = intArrayOf(
        ...
        0x1F6CC, 0x1F6CC, 0x1F6D0, 0x1F6D2, 0x1F6EB, 0x1F6EC, 0x1F6F4, 0x1F6F9,
        0x1F910, 0x1F93E, 0x1F940, 0x1F970, 0x1F973, 0x1F976, 0x1F97A, 0x1F97A,
